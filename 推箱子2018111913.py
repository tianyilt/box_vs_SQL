...
#五个基本函数
def scores():
    label_1.config(text="current score:%d" %score[0])
    label_2.config(text="high score:%d" %score[1])
def erased(i,j):
    t=c.create_image(32*i+16,32*j+16,image=bg_img)
def host(i,j):
...
#说明
def explain():
    showinfo(title="游戏说明",message="使用键盘的上下左右或wasd键,操控小人,将箱子推到指定的终点即可完成游戏")
#重开
def re():
    if current_mode[0] == "2":
        w_high()
...
Menubar.add_cascade(label="难度选择",menu=diffculty)
diffculty.add_command(label="低",command=w_low)
diffculty.add_command(label="中",command=w_medium)
diffculty.add_command(label="高",command=w_high)
restart=Menu(Menubar,tearoff=0)
Menubar.add_command(label="重新开始",command=re)
Menubar.add_command(label="游戏说明",command=explain)
Menubar.add_command(label="退出",command=quit)
Menubar.add_command(label="清除记录",command=unregister)
